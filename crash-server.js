...
function handleClientMessage(message) {
  ...
  switch (type) {
    case "bet":
      const { userId: betUserId, value } = data;
      if (gameState.phase === "betting" && betUserId && value > 0) {
        gameState.bets.push({
          userId: betUserId,
          amount: value,
        ...
        broadcastGameState();
        sendBetAddedEvent(betUserId, value);
      }
      break;

    case "cashout":
      const { userId: cashoutUserId } = data;
      console.log("Received cashout request:", {
        cashoutUserId,
        data,
        message,
      });
      const userBet = gameState.bets.find((u) => u.userId == cashoutUserId);
      console.log("Found user bet:", userBet);
      if (gameState.phase === "running" && userBet) {
        userBet.prize = userBet.amount * gameState.multiplier;
        console.log("User cashed out successfully:", userBet);
        broadcastGameState();
      } else {
        console.log(
          "Cashout failed - Phase:",
          gameState.phase,
          "User bet found:",
          !!userBet,
        );
